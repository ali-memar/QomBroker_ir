...
(function($) {
    ...
    $("#requestForm").validator().on("submit", function(event) {
    	if (event.isDefaultPrevented()) {
            ...
            rformError();
            rsubmitMSG(false, "لطفا همه فیلد ها را پر کنید");
        } else {
    ...
    function rformSuccess() {
        $("#requestForm")[0].reset();
        rsubmitMSG(true, "درخواست ارسال شد");
    ...
    $("#contactForm").validator().on("submit", function(event) {
    	if (event.isDefaultPrevented()) {
            // handle the invalid form...
            cformError();
            csubmitMSG(false, "لطفا همه فیلد ها را پر کنید");
        } else {
            // everything looks good!
            event.preventDefault();
            csubmitForm();
    ...
    $("#privacyForm").validator().on("submit", function(event) {
    	if (event.isDefaultPrevented()) {
            // handle the invalid form...
            pformError();
            psubmitMSG(false, "لطفا همه فیلد ها را پر کنید");
        } else {
            // everything looks good!
            event.preventDefault();
            psubmitForm();
    ...
    function pformSuccess() {
        $("#privacyForm")[0].reset();
        psubmitMSG(true, "درخواست اراسل شد");
        $("input").removeClass('notEmpty'); // resets the field label after submission
    }
